import { getRecentTracks } from "@/actions/tracks";
    ...
    <div className="grid grid-flow-row grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3">
      {tracks.map((item) => (
        ...
        >
          <Link
            href={`/tracks/${item.id}`}
            className="inline-block overflow-hidden max-w-[60%] md:max-w-[70%]"
          >
            ...
            <Image
              ...
              alt="track-image"
              className="absolute top-0 left-0 z-0 -translate-y-[50%] filter-[contrast(200%)]"
              height={1000}
              width={1000}
              objectFit="cover"
              src={item.image}
